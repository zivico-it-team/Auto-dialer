...
2. Use Node.js **20 LTS or newer**, production mode, and set the entry file to `app.js`. The backend `postinstall` script generates Prisma client code, applies the Prisma schema, and compiles `src/server.ts` to `dist/server.js` automatically during Hostinger's dependency installation.
